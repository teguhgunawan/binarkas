<?php
$transactionFormData = $transactionFormData ?? ['date' => date('Y-m-d'), 'title' => '', 'account_name' => '', 'category' => '', 'type' => 'expense', 'amount' => '0'];
...
<section class="surface-card mb-4">
    ...
    <form method="post" action="<?= htmlspecialchars(app_base('?page=transactions')) ?>">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-2"><label class="form-label">Account</label><select name="account_name" class="form-select rounded-4 <?= isset($transactionFormErrors['account_name']) ? 'is-invalid' : '' ?>"><?php foreach ($accounts as $account): ?><option value="<?= htmlspecialchars((string) ($account['name'] ?? '')) ?>" <?= ($transactionFormData['account_name'] ?? '') === ($account['name'] ?? '') ? 'selected' : '' ?>><?= htmlspecialchars((string) ($account['name'] ?? '')) ?></option><?php endforeach; ?></select><?php if (isset($transactionFormErrors['account_name'])): ?><div class="invalid-feedback"><?= htmlspecialchars($transactionFormErrors['account_name']) ?></div><?php endif; ?></div>
...
<section class="surface-card">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4"><h3 class="h5 mb-0">Transaction Ledger</h3><div class="text-muted small">All records</div></div>
        ...
        <?php foreach ($transactions as $item): ?>
            <div class="rounded-4 bg-light-subtle p-3">
                <form method="post" action="<?= htmlspecialchars(app_base('?page=transactions')) ?>" class="row g-3 align-items-end">
                    ...
                    <div class="col-lg-2"><label class="form-label small">Account</label><select name="account_name" class="form-select rounded-4"><?php foreach ($accounts as $account): ?><option value="<?= htmlspecialchars((string) ($account['name'] ?? '')) ?>" <?= ($item['account_name'] ?? '') === ($account['name'] ?? '') ? 'selected' : '' ?>><?= htmlspecialchars((string) ($account['name'] ?? '')) ?></option><?php endforeach; ?></select></div>
                    ...
                    <div class="col-lg-1 d-grid"><button type="submit" class="btn btn-outline-primary rounded-pill">Save</button></div>
                </form>
